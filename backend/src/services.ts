...
export async function extractAnnotations(menuId: string) {
...
function splitMenuLine(text: string) {
  ...
  if (!priceMatch) return [{ text, type: detectLabel(text) }];
...
export async function ExtractMenuItems(menuId: string) {
...
function detectLabel(text: string) {
  if (/^\$?\d+(\.\d{2})?$/.test(text)) return "price";
  if (text.length > 20) return "description";
  if (/APPETIZERS|ENTREES|DRINKS|DESSERTS/i.test(text)) return "category";
  return "item";
}
